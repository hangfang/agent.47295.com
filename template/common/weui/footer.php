        ...
        refresh: function(){
            try{
                ...
                if(number>0){
                    ...
                    tmp.length && tmp.html('成本:'+price).show();
                }else if(number==0){
                    $('.cart_product_num').html(0).hide();
                    var tmp = $('.cart_number_total');
                    tmp.length && tmp.html('0件').show();
                    var tmp = $('.cart_price_total');
                    tmp.length && tmp.html('成本:0').show();
                }
                return number;
            }catch(e){
    ...
    $(function(){
        ...
        $('#shop_entry_btn').click(function(){
            var menu = [
                ...
                {'url':'<?php echo STATIC_CDN_URL.'static/shop/images/qrcode_for_gh_a103c9f558fa_258.jpg'?>','text':'关注我们'},
            ];
