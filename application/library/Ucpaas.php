...
class Ucpaas
{
    ...
    private $resp_msg = array(
        ...
        105136=>'自签短信签名长度为2到12位汉字、英文字母和数字，不能纯数字',
        105137=>'群发短信全部失败',
        105138=>'群发短信号码不能重复',
        105139=>'补发短信模板不规范',
        105147=>'对同个号码的发送频率超过限制，如有特殊需要请联系客服'
    );
    ...
    private function getResult($url, $body = null, $type = 'json',$method){
        ...
        if (isset($data) && !empty($data)) {
            ...
            if($result->resp->respCode!='000000'){
                $result->resp->respMsg = isset($this->resp_msg[$result->resp->respCode]) ? $this->resp_msg[$result->resp->respCode] : '操作失败';
            }
        } else {
            $result = '没有返回数据';
        }
        
        log_message('info', 'Ucpaas request url: '.$url.', post body: '.$body.', response: '.json_encode($result, JSON_UNESCAPED_UNICODE));
        return $result;
    }
    ...
    public function applyClient($appId, $clientType, $charge, $friendlyName, $mobile, $type = 'json'){
        ...
        return $this->getResult($url, $body, $type,'post');
    }
     ...
     * @param $tContent
     * @param $tSign '诸葛到店'
     * @param $tOpt 0-通知 4-验证码 5-营销类
     * @return mixed|string
     * @links http://www.ucpaas.com/maap/sms/addTemplate
     */
    public function createTemplate($appId, $tName, $tContent, $tSign = "诸葛到店", $tOpt = 0, $type='json') {
        if ($type == 'json') {
            $type = 'json';
        } elseif ($type == 'xml') {
            $type = 'xml';
        ...
        $url = self::BaseUrl . self::SoftVersion . '/Accounts/' . $this->accountSid . '/Messages/addsmstemplate?sig=' . $this->getSigParameter();
        
        $params = $template = array();
        $template['appId'] = $appId;
        $template['type'] = $tOpt;
        $template['templateName'] = $tName;
        $template['autograph'] = self::AUTOGRAPH;
        $template['content'] = $tContent;
        $params = array('template'=>$template);
        return $this->getResult($url, json_encode($params), $type, 'post');
    }
     ...
     * @param $tOpt 0-通知 4-验证码 5-营销类
     * @return mixed|string
     * @links http://www.ucpaas.com/maap/sms/editTemplate
     */
    public function editTemplate($appId, $templateId, $tName, $tContent, $tSign = "诸葛到店", $tOpt = 0, $type='json') {
        if ($type == 'json') {
            $type = 'json';
        } elseif ($type == 'xml') {
            $type = 'xml';
        ...
        $url = self::BaseUrl . self::SoftVersion . '/Accounts/' . $this->accountSid . '/Messages/editsmstemplate?sig=' . $this->getSigParameter();
     ...
     * @param $templateId
     * @return mixed|string
     * @links http://www.ucpaas.com/maap/sms/getTemplate
     */
    public function getTemplate($appId, $templateId, $type='json') {
        if ($type == 'json') {
            $type = 'json';
        } elseif ($type == 'xml') {
            $type = 'xml';
        ...
        $url = self::BaseUrl . self::SoftVersion . '/Accounts/' . $this->accountSid . '/Messages/getsmstemplate?sig=' . $this->getSigParameter();
        return $this->getResult($url, json_encode(array('template'=>array('templateId'=>$templateId))), $type, 'post');
    }
     ...
     * @param $templateId
     * @return mixed|string
     * @links http://www.ucpaas.com/maap/sms/delTemplate
     */
    public function delTemplate($appId, $templateId, $type='json') {
        if ($type == 'json') {
            $type = 'json';
        } elseif ($type == 'xml') {
            $type = 'xml';
        ...
        $url = self::BaseUrl . self::SoftVersion . '/Accounts/' . $this->accountSid . '/Messages/deletesmstemplate?sig=' . $this->getSigParameter();
        return $this->getResult($url, json_encode(array('template'=>array('templateId'=>$templateId))), $type, 'post');
    }
}